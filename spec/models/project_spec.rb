...
RSpec.describe Project do
  ...
  xit "knows that a project with an incomplete task is not done" do
    project.tasks << task.complete?
    expect(project).to be(false)
  end

  it "marks a project done if the tasks are done" do
    project.tasks << task
    task.mark_completed
    expect(project).to be_done
  end
  ...
  it "properly handles a blank project" do
    ...
    expect(project.project_days_remaining).to be_nan
  ...
  describe "estimates" do
    ...
    it "knows its project days remaning" do
      expect(project.project_days_remaining).to eq(35)
    end
